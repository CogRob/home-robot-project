...
class SRPipeline:
    ...
    def runner(self):
        """ """
        move_goal = moveRobotBaseGoal(pose=Pose2D(-0.01, -2.240, 0))
        rospy.loginfo("Created goal")
        self.move_fetch_base_client.send_goal_and_wait(move_goal)
        rospy.loginfo("Sent goal")

        misplaced_objects_response = self.tidy_module_client(
            IdentifyMisplacedObjectsRequest()
        ).object_locations
        ...
        for misplaced_object in misplaced_objects_response:
            ...
            placement_candidates = self.correct_object_placement_client(
                GetCorrectPlacementsRequest(misplaced_object)
            ).placements
            ...
            for room_receptacles in placement_candidates.candidates:
                ...
                self.object_room_navigate_action_client.send_goal_and_wait(
                    NavigateToRoomGoal(room=room_receptacles.room)
                )
                print("Navigating to receptacles ", room_receptacles.receptacles)
                self.receptacle_navigate_action_client.send_goal_and_wait(
                    NavigateToReceptaclesGoal(receptacles=room_receptacles.receptacles)
                )
                break
