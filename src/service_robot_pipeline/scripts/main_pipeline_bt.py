...
def create_bt():
    ...
    place_obj_on_receptacle_behavior = py_trees.composites.Sequence(
        "PlaceObjOnReceptacle", memory=True
    )
    place_obj_on_receptacle_behavior.add_children(
        [
            ...
            PrepareToActuateBehavior(),
            NavigateToReceptacleBehavior(blackboard_key="target_receptacle"),
            place_behavior_complex,
        ]
    )

    place_object_in_room_receptacle_behavior = py_trees.composites.Sequence(
        "Put Away Object in Correct Location", memory=True
    )
    place_object_in_room_receptacle_behavior.add_children(
        [
            ...
            NavigateToRoomBehavior(in_blackboard_key="target_placement"),
            GetReceptaclesLocationBehavior(
    ...
    tidy_seq.add_children(
        [
            ...
            PickupBehavior(blackboard_key="misplaced_object"),
            GetPlacementCandidatesBehavior(
    ...
    tidy_bt.add_children(
        [
            ...
            # ),
            IDMisplacedObjectBehavior(out_blackboard_key="misplaced_objects"),
            RepeatUntilSuccessDecorator(
                name="TidyRoomDec",
                child=tidy_seq,
