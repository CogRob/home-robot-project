...
class ServiceClient(py_trees.behaviour.Behaviour):
    ...
    * Trigger true pre-emption by sending a new goal in :meth:`~py_trees_ros.actions.ActionClient.update()`
    ...
    """
    def __init__(self, name="Service Client", action_spec=None, action_goal=None, action_namespace="/action",
                 override_feedback_message_on_running="moving"):
        super(ServiceClient, self).__init__(name)
    ...
    def setup(self, timeout = None):
        ...
        rospy.wait_for_service(self.action_namespace)
        self.action_client = rospy.ServiceProxy(
    ...
    def update(self):
        ...
        if not self.sent_goal:
            self.action_result = self.action_client(self.action_goal)
            self.sent_goal = True
            self.feedback_message = "sent goal to the service server"
            self.status = py_trees.Status.RUNNING
            return py_trees.Status.RUNNING

        self.feedback_message = "Sent goal"
...
class FromBlackBoard(ServiceClient):
    def __init__(self, name="Action Client", action_spec=None, action_goal=None, blackboard_key = "", action_namespace="/action",
                 override_feedback_message_on_running="moving"):
        

        super(FromBlackBoard, self).__init__(
            name = name,
    ...
    #     self.action_goal = self.blackboard.goal
    #     super(FromBlackBoard, self).initialise()
...
class FromConstant(FromBlackBoard):
    def __init__(self, name="Action Client", action_spec=None, action_goal=None, action_namespace="/action",
        ...
        self.blackboard_key = "/goal" + str(self.id)
        super(FromConstant, self).__init__(
            name = name,
            action_spec = action_spec,
            action_goal = action_goal,
            action_namespace = action_namespace,
            override_feedback_message_on_running = override_feedback_message_on_running,
            blackboard_key=self.blackboard_key,
        )
        self.blackboard.register_key(
            key=self.blackboard_key,
            access=py_trees.common.Access.WRITE,
        )
        self.blackboard.set(name=self.blackboard_key, value=action_goal)
