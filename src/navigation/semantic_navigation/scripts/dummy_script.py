...
from semantic_navigation.msg import objectNavigateAction, objectNavigateGoal, objectNavigateResult
from local_path_planner.msg import moveRobotBaseActionGoal, moveRobotBaseAction
import random
...
class ObjectNavigation(object):
    def __init__(self):
        self._as = actionlib.SimpleActionServer("object_navigate_action_server", objectNavigateGoal, execute_cb=self.semantic_navigate_cb, auto_start = False)
        self._as.start()
        ...
        self.as_result = objectNavigateResult()
    ...
    def semantic_navigate_cb(self, request):
        ...
        goto_keypoint_goal = moveRobotBaseActionGoal()
        ...
        # Change this
        while True:
            goto_keypoint_goal.pose = Pose2D(random.random() * 10, random.random() * 10, random.random() * 3.14)
            self.goto_kp_client.send_goal(goto_keypoint_goal)
            self.goto_kp_client.wait_for_result()

            if random.choice([True, False]):
                self.self.as_result = random.choice([True, False])
                break
