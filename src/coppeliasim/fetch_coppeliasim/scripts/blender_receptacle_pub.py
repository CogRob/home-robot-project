...
class ImagePublisher:
    def __init__(self, topic='blender_camera/image_receptacle_mask', rate=10):
        ...
        self.receptacles  = {'sofa' : 1000, 'cabinet' : 1001, 'table_living_rooom' : 1002}
        self.load_receptacle_ids()

        self.load_camera()

    def load_receptacle_ids(self):
        for object_name in self.receptacles.keys():
            collection = bpy.data.collections.get(object_name)
            for obj in collection.objects:
                obj['inst_id'] = self.receptacles[object_name]
    ...
    def load_robot_arm(self):
        ...
        for link_name in self.robot_arm_link_names:
            if 'gripper' in link_name:
                bpy.ops.import_scene.obj(filepath='/root/home-robot-project/src/fetch/fetch_ros/fetch_description/meshes/%s.obj'%(link_name))
            else:
                bpy.ops.wm.collada_import(filepath='/root/home-robot-project/src/fetch/fetch_ros/fetch_description/meshes/%s.dae'%(link_name))
    ...
    #         bpy.ops.import_scene.obj(filepath='/root/home-robot-project/src/objects_description/%s/textured_simple.obj'%(object_name))
    ...
    def publish_image(self):
        while not rospy.is_shutdown():
            ...
            try:
                ...
                ros_image = self.bridge.cv2_to_imgmsg(result.vis(), encoding="passthrough")
